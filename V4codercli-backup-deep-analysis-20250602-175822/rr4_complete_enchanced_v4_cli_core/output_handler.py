...
class OutputHandler:
    ...
    def save_collection_report(self, summary: Optional[Dict[str, Any]] = None) -> None:
        """Save collection report to file.
        
        Args:
            summary: Optional summary dictionary. If not provided, generates one.
        """
        try:
            if summary is None:
                summary = self.get_run_summary()
            
            # Handle different summary formats - ensure we have run_directory
            if 'run_directory' not in summary:
                summary['run_directory'] = str(self.current_run_dir)
            
            # Save JSON report
            report_file = self.current_run_dir / "collection_report.json"
            with open(report_file, 'w') as f:
                json.dump(summary, f, indent=2, default=str)
            
            # Generate human-readable report
            text_report_file = self.current_run_dir / "collection_report.txt"
            with open(text_report_file, 'w') as f:
                f.write("="*70 + "\n")
                f.write("RR4 COLLECTION REPORT\n")
                f.write("="*70 + "\n\n")
                
                f.write(f"Run Directory: {summary.get('run_directory', 'N/A')}\n")
                f.write(f"Run ID: {summary.get('run_id', 'N/A')}\n")
                f.write(f"Timestamp: {summary.get('timestamp', 'N/A')}\n")
                
                # Handle different structures
                if 'devices' in summary:
                    # OutputHandler format
                    f.write(f"Total Devices: {len(summary['devices'])}\n")
                    f.write(f"Total Layers: {len(summary.get('layers', []))}\n")
                    f.write(f"Total Commands: {summary.get('total_commands', 0)}\n")
                    f.write(f"Total Errors: {summary.get('errors', 0)}\n\n")
                    
                    f.write("DEVICE BREAKDOWN:\n")
                    f.write("-" * 50 + "\n")
                    for device in summary['devices']:
                        f.write(f"  {device['hostname']}:\n")
                        f.write(f"    Layers: {', '.join(device.get('layers', []))}\n")
                        f.write(f"    Commands: {device.get('commands', 0)}\n")
                        f.write(f"    Errors: {device.get('errors', 0)}\n\n")
                elif 'device_results' in summary:
                    # Main script format
                    f.write(f"Total Devices: {len(summary['device_results'])}\n")
                    f.write(f"Summary: {summary.get('summary', {})}\n\n")
                    
                    f.write("DEVICE BREAKDOWN:\n")
                    f.write("-" * 50 + "\n")
                    for hostname, device_data in summary['device_results'].items():
                        f.write(f"  {hostname}:\n")
                        f.write(f"    Success Count: {device_data.get('success_count', 0)}\n")
                        f.write(f"    Failure Count: {device_data.get('failure_count', 0)}\n")
                        f.write(f"    Tasks: {len(device_data.get('tasks', []))}\n\n")
            
            self.logger.info(f"Collection report saved to {report_file}")
            
        except Exception as e:
            self.logger.error(f"Failed to save collection report: {e}")
            # Don't re-raise since this is not critical
