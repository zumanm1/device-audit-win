...
SETUP_LOGGING_DIAG_FILE = "setup_logging_diag.txt" # New diagnostic file

def setup_logging():
    """Configures logging to file and console, with enhanced diagnostics."""
    abs_log_file_path = os.path.abspath(LOG_FILE_NAME)
    abs_diag_file_path = os.path.abspath(SETUP_LOGGING_DIAG_FILE)
    diag_messages = []

    diag_messages.append(f"[DEBUG] Attempting to configure logging. Target log file: {abs_log_file_path}")
    print(diag_messages[-1]) # Print to console
    ...
    try:
        ...
        diag_messages.append(f"[DEBUG] logging.basicConfig called for {abs_log_file_path}")
        print(diag_messages[-1])
        
        if os.path.exists(abs_log_file_path):
            diag_messages.append(f"[SUCCESS] Log file confirmed to exist after basicConfig: {abs_log_file_path}")
            print(diag_messages[-1])
            logging.info("Logging configured successfully. Log file: %s", abs_log_file_path)
        else:
            err_msg = f"[ERROR] Log file DOES NOT exist after basicConfig: {abs_log_file_path}"
            diag_messages.append(err_msg)
            print(err_msg)
            # logging.error might not work if basicConfig failed to set up file handler
            # Try to write to our separate diagnostic file
            try:
                with open(abs_diag_file_path, 'a') as f_diag:
                    f_diag.write(f"{datetime.datetime.now()}: {err_msg}\n")
                    f_diag.write("Attempting to write to primary log via logging.error (may fail):\n")
                logging.error("Failed to create or access log file after basicConfig: %s", abs_log_file_path)
            except Exception as e_diag_write:
                print(f"[CRITICAL] Could not even write to diagnostic file {abs_diag_file_path}: {e_diag_write}")

    except Exception as e:
        crit_err_msg = f"[CRITICAL] Exception during logging.basicConfig setup: {e}"
        diag_messages.append(crit_err_msg)
        print(crit_err_msg)
        import sys
        sys.stderr.write(f"CRITICAL LOGGING SETUP ERROR: {e}\n")
        try:
            with open(abs_diag_file_path, 'a') as f_diag:
                f_diag.write(f"{datetime.datetime.now()}: {crit_err_msg}\nDetails: {type(e).__name__} - {str(e)}\n")
        except Exception as e_diag_write_crit:
            print(f"[CRITICAL] Could not write critical error to diagnostic file {abs_diag_file_path}: {e_diag_write_crit}")
