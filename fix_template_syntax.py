...
if extra_endif_line - 1 < len(lines) and '{% endif %}' in lines[extra_endif_line - 1]:
    ...
    print(f"Removed extra {% endif %} from line {extra_endif_line}")
else:
    print(f"Warning: Expected extra {% endif %} at line {extra_endif_line} not found. File may have changed.")
...
if adjusted_position < len(lines):
    ...
    print(f"Added missing {% endblock %} after line {missing_endblock_position - 1}")
else:
