...
class ConnectivityAuditor:
    ...
    def audit_device(self, device):
        ...
        for port in device['check_ports']:
            ...
            if port_result['open']:
                print(f"    {Fore.GREEN}✓ Port {port} open (response time: {port_result['response_time_ms']:.2f} ms){Style.RESET_ALL}")
            else:
                print(f"    {Fore.RED}✗ Port {port} closed ({port_result['error']}){Style.RESET_ALL}")
